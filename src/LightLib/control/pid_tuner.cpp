...
// Upper caps per [PidType][slot] — reached by holding the [+] button. Stops
// the user from running gains to wildly unstable values that would saturate
// motor commands the moment auton runs. dec_cb already clamps at 0.
static constexpr double MAX[light::PID_COUNT][light::CONST_COUNT] = {
    /* DRIVE   */ {10.0, 1.0, 5.0, 200.0},
    /* TURN    */ {10.0, 1.0, 5.0, 200.0},
    /* SWING   */ {10.0, 1.0, 5.0, 200.0},
    /* HEADING */ {10.0, 1.0, 5.0, 200.0},
    /* EKF     */ {0.5, 0.05, 10.0, 50.0},
};
...
static inline int unpack_ud(void* ud) {
  return (int)((intptr_t)ud & 0xF);
}
...
void PidTuner::set_drive(light::Drive* drive) {
  drive_ = drive;
  if (!drive_) return;
  auto load = [&](PidType t, light::PID::Constants c) {
...
void PidTuner::build_ui() {
  ...
  for (int i = 0; i < CONST_COUNT; i++) {
    ...
    lv_obj_set_user_data(dec, pack_ud(i));
    ...
    lv_obj_set_user_data(inc, pack_ud(i));
...
void PidTuner::select_pid(PidType t) {
  ...
  for (int i = 0; i < PID_COUNT; i++) {
    if (i == (int)t)
      lv_obj_add_state(tab_btns_[i], LV_STATE_CHECKED);
    else
      lv_obj_clear_state(tab_btns_[i], LV_STATE_CHECKED);
  }
...
void PidTuner::push_sample() {
  ...
  auto note = [&](double v) {
    if (v > chart_y_max_) {
      chart_y_max_ = std::ceil(v / 10.0) * 10.0;
    }
    if (v < chart_y_min_) {
      chart_y_min_ = std::floor(v / 10.0) * 10.0;
    }
  };
  note(lv);
  note(rv);
  ...
  if (chart_y_max_ != last_chart_y_max_ || chart_y_min_ != last_chart_y_min_) {
    lv_chart_set_range(chart_, LV_CHART_AXIS_PRIMARY_Y,
                       (lv_coord_t)chart_y_min_, (lv_coord_t)chart_y_max_);
    last_chart_y_max_ = chart_y_max_;
    last_chart_y_min_ = chart_y_min_;
  }

  lv_chart_set_next_value(chart_, ser_left_, cl(lv));
  lv_chart_set_next_value(chart_, ser_right_, cl(rv));
  lv_chart_set_next_value(chart_, ser_err_, cl(ev));
...
void PidTuner::inc_cb(lv_event_t* e) {
  ...
  double cap = MAX[self->active_pid_][slot];
  if (v > cap) v = cap;
...
void PidTuner::record_cb(lv_event_t* e) {
  ...
  if (self->recording_) {
    ...
    self->chart_y_max_ = 1.0;
    self->chart_y_min_ = -1.0;
    lv_chart_set_range(self->chart_, LV_CHART_AXIS_PRIMARY_Y,
                       (lv_coord_t)self->chart_y_min_, (lv_coord_t)self->chart_y_max_);
