...
void Drive::pid_angle_behavior_bias_set(e_angle_behavior behavior) {
  if (behavior == light::LEFT_TURN)
    turn_biased_left = true;
  else if (behavior == light::RIGHT_TURN)
    turn_biased_left = false;
  else
    printf("Must input 'left' or 'right' for angle behavior bias!\n");
}
e_angle_behavior Drive::pid_angle_behavior_bias_get() { return turn_biased_left ? light::LEFT_TURN : light::RIGHT_TURN; }
...
//  - longest
//  - raw
void Drive::pid_angle_behavior_set(light::e_angle_behavior behavior) {
