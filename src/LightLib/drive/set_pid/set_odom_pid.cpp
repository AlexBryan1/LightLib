...
void Drive::pid_odom_behavior_set(light::e_angle_behavior behavior) { default_odom_type = behavior; }
light::e_angle_behavior Drive::pid_odom_behavior_get() { return default_odom_type; }
...
/////
// No units
void Drive::pid_odom_injected_pp_set(std::vector<light::odom> imovements) {
  bool slew_on = imovements[0].drive_direction == fwd ? slew_drive_forward_get() : slew_drive_backward_get();
  pid_odom_injected_pp_set(imovements, slew_on);
}
void Drive::pid_odom_injected_pp_set(std::vector<light::odom> imovements, bool slew_on) {
  xyPID.timers_reset();
  current_a_odomPID.timers_reset();
...
// Units
void Drive::pid_odom_injected_pp_set(std::vector<light::united_odom> p_imovements) {
  std::vector<odom> imovements = util::united_odoms_to_odoms(p_imovements);
  pid_odom_injected_pp_set(imovements);
}
void Drive::pid_odom_injected_pp_set(std::vector<light::united_odom> p_imovements, bool slew_on) {
  std::vector<odom> imovements = util::united_odoms_to_odoms(p_imovements);
  pid_odom_injected_pp_set(imovements, slew_on);
}
...
void Drive::raw_pid_odom_pp_set(std::vector<odom> imovements, bool slew_on) {
  // Guard: empty path has nothing to do; size==1 has no "second-to-last"
  // (size()-2 underflows size_t and reads out-of-bounds).
  if (imovements.empty()) return;
  odom_second_to_last = imovements.size() >= 2
                            ? imovements[imovements.size() - 2].target
                            : imovements[0].target;
