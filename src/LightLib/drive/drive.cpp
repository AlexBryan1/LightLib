...
void Drive::drive_defaults_set() {
  ...
  pid_odom_behavior_set(light::shortest);  // Default odom turning to shortest
...
// Refresh the cached TICK_PER_INCH whenever ratio or cartridge change so
// drive_sensor_left/right can read the cached value instead of recomputing
// circumference and tick-per-rev on every sensor poll.
...
double Drive::drive_sensor_right() {
  ...
  // Use the cached TICK_PER_INCH instead of recomputing it (refreshed by
  // drive_ratio_set / drive_rpm_set / wheel-diameter setters).
  return drive_sensor_right_raw() / TICK_PER_INCH;
}
...
double Drive::drive_sensor_left() {
  ...
  return drive_sensor_left_raw() / TICK_PER_INCH;
}
