...
static std::map<std::string, bool> g_autotunePrevSuccess;
  ...
  while ((int)(pros::millis() - t0) < maxMotionMs) {
    ...
    if (vL < settleVoltMv && vR < settleVoltMv) {
      if (!isCurrentlyStopped) {
        ...
        // One complete "good stop" window achieved
        consecutiveStops++;
        printf("[AUTOTUNE] Good stop confirmed (%d/3)\n", consecutiveStops);
        
        if (consecutiveStops >= 3) {
          break; // Successfully exited after 3 solid stops in a row
        }
        // Reset tracking window to look for the next consecutive stop window
        currentStopStart = now; 
      }
    } else {
      // Robot broke stability window; wipe the streak completely
...
static void runVelocityAutotune(light::Drive* chassis,
                                ...
                                int maxItersPerPhase, // Removed kpStart parameter
