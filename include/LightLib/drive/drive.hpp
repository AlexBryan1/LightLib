...
class Drive {
 ...
 private:
  ...
  double new_turn_target_compute(double target, double current, light::e_angle_behavior behavior);
  ...
  std::pair<float, float> decide_vert_sensor(light::tracking_wheel* tracker, bool is_tracker_enabled, float ime = 0.0, float ime_track = 0.0);
