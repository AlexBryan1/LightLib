...
struct PathEntry {
    const char* name; ///< Lookup name passed to `light::runPath()`.
    const char* csv;  ///< Pointer to the static CSV blob.
};

/** Full path registry. Used by `light::runPath()` to resolve names. */
