...
class RotationalSnap {
 public:
  ...
  //
  // INVARIANT: must be called every tick while the controller is connected.
  // If a tick is missed between non-zero and zero input, the latch may be
  // skipped (the snap target won't engage until the next non-zero → zero
  // transition).
