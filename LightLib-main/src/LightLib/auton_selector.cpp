...
static void sel_unpack(void* ud, int& slot, int& sign) {
...
void AutonSelector::teardown_ui() {
  ...
  if (run_toggle_lbl_) {
    lv_obj_del(run_toggle_lbl_);
    run_toggle_lbl_ = nullptr;
  }
  if (run_screen_) {
    lv_obj_del(run_screen_);
    run_screen_ = nullptr;
  }
  if (screen_) {
    lv_obj_del(screen_);
  ...
  btn_objs_.clear();
  for (int i = 0; i < 4; ++i) {
    pid_tab_btns_[i] = nullptr;
    for (int s = 0; s < 4; ++s) pid_val_labels_[i][s] = nullptr;
  }
}
...
void AutonSelector::build_right_pid(lv_obj_t* parent) {
  ...
  for (int s = 0; s < 4; s++) {
    ...
    lv_obj_set_user_data(dec, sel_pack(s, -1));  // pid is read live from active_pid_tab_
    ...
    lv_obj_set_user_data(inc, sel_pack(s, +1));
...
void AutonSelector::select_pid_tab(int idx) {
  ...
  for (int i = 0; i < 4; i++) {
    if (i == idx)
      lv_obj_add_state(pid_tab_btns_[i], LV_STATE_CHECKED);
    else
      lv_obj_clear_state(pid_tab_btns_[i], LV_STATE_CHECKED);
    for (int s = 0; s < 4; s++) {
      if (pid_val_labels_[i][s]) {
        if (i == idx)
          lv_obj_clear_flag(pid_val_labels_[i][s], LV_OBJ_FLAG_HIDDEN);
        else
          lv_obj_add_flag(pid_val_labels_[i][s], LV_OBJ_FLAG_HIDDEN);
      }
    }
  }
}
...
void AutonSelector::init_pid_vals() {
  ...
  if (!drive) return;
  auto load = [this](int t, light::PID::Constants c) {
    pid_vals_[t][0] = c.kp;
    pid_vals_[t][1] = c.ki;
    pid_vals_[t][2] = c.kd;
...
void AutonSelector::apply_pid_vals() {
  ...
  switch (active_pid_tab_) {
    case 0:
      drive->pid_drive_constants_set(v[0], v[1], v[2], v[3]);
      break;
    case 1:
      drive->pid_turn_constants_set(v[0], v[1], v[2], v[3]);
      break;
    case 2:
      drive->pid_swing_constants_set(v[0], v[1], v[2], v[3]);
      break;
    case 3:
      drive->pid_heading_constants_set(v[0], v[1], v[2], v[3]);
      break;
    default:
      break;
  }
...
void AutonSelector::select(int idx) {
  ...
  for (int i = 0; i < (int)btn_objs_.size(); i++) {
    if (i == idx)
      lv_obj_add_state(btn_objs_[i], LV_STATE_CHECKED);
    else
      lv_obj_clear_state(btn_objs_[i], LV_STATE_CHECKED);
  }
  selected_idx_ = idx;
}
...
void AutonSelector::odom_timer_cb(lv_timer_t* timer) {
  ...
  if (strcmp(buf, self->odom_x_last_) != 0) {
    strncpy(self->odom_x_last_, buf, sizeof(self->odom_x_last_) - 1);
    lv_label_set_text(self->odom_x_lbl_, buf);
  }
  snprintf(buf, sizeof(buf), "%.2f in", p.y);
  if (strcmp(buf, self->odom_y_last_) != 0) {
    strncpy(self->odom_y_last_, buf, sizeof(self->odom_y_last_) - 1);
    lv_label_set_text(self->odom_y_lbl_, buf);
  }
  snprintf(buf, sizeof(buf), "%.1f deg", p.theta);
  if (strcmp(buf, self->odom_th_last_) != 0) {
    strncpy(self->odom_th_last_, buf, sizeof(self->odom_th_last_) - 1);
...
void AutonSelector::pid_adj_cb(lv_event_t* e) {
  auto* self = static_cast<AutonSelector*>(lv_event_get_user_data(e));
  int slot, sign;
  sel_unpack(lv_obj_get_user_data(lv_event_get_target(e)), slot, sign);
