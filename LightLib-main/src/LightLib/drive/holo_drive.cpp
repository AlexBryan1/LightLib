...
void HoloDrive::set_holonomic(double throttle, double strafe, double turn) {
  ...
  // Round (don't truncate) so the proportional scale-down doesn't lose
  // 0–4 cumulative units across the four motors. Then skip the smartport
  // write when the value matches what we last sent.
...
// Free function — shared by HoloDrive, HDrive, and rotational_snap.
// fmod handles arbitrary-magnitude inputs (e.g. pros::Rotation accumulating
// across many turns) in O(1) instead of looping.
