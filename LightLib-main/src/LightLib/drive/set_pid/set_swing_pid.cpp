...
bool Drive::is_swing_slew_enabled(e_swing type, double target, double current) {
  int side = type == light::LEFT_SWING ? 1 : -1;
...
void Drive::pid_swing_behavior_set(light::e_angle_behavior behavior) { default_swing_type = behavior; }
light::e_angle_behavior Drive::pid_swing_behavior_get() { return default_swing_type; }
...
void Drive::pid_swing_set(e_swing type, double target, int speed, int opposite_speed, e_angle_behavior behavior, bool slew_on) {
  ...
  if (odom_theta_direction_get())
    current_swing = current_swing == light::LEFT_SWING ? light::RIGHT_SWING : light::LEFT_SWING;
