...
void Drive::opcontrol_curve_sd_initialize() {
  ...
  if (!light::util::SD_CARD_ACTIVE) return;
...
void Drive::button_press(button_* input_name, int button, std::function<void()> change_curve, std::function<void()> save) {
  ...
  // Then, increase the curve every 100ms by 0.1
    ...
    input_name->hold_timer += light::util::DELAY_TIME;
    if (input_name->hold_timer > 500.0) {
      input_name->increase_timer += light::util::DELAY_TIME;
  ...
  // When button is released for 250ms, save the new curve value to the SD card
    ...
    if (input_name->release_reset) {
      input_name->release_timer += light::util::DELAY_TIME;
...
double Drive::opcontrol_curve_left(double x) {
  if (left_curve_scale != 0) {
    // expf(z) is the actual base-e exp; powf(2.718, z) was a slower approximation.
    const float k = expf(-(left_curve_scale / 10.0f));
    return (k + expf((fabs(x) - 127.0f) / 10.0f) * (1.0f - k)) * x;
  }
  return x;
}
...
double Drive::opcontrol_curve_right(double x) {
  if (right_curve_scale != 0) {
    const float k = expf(-(right_curve_scale / 10.0f));
    return (k + expf((fabs(x) - 127.0f) / 10.0f) * (1.0f - k)) * x;
  }
  return x;
}
...
void Drive::opcontrol_tank() {
  ...
  int r_stick = opcontrol_curve_right(clipped_joystick(master.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_Y)));
