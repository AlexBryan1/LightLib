...
void Drive::pid_tuner_brain_init() {
  last_auton_selector_state = false;
  pros::lcd::initialize();
}
...
void Drive::pid_tuner_print() {
  ...
  // Single snprintf into a stack buffer instead of chained std::string + —
  // saves ~6 heap allocs per call (this runs every opcontrol tick while open).
  const char* a = arrow.c_str();
  char buf[256];
  snprintf(buf, sizeof(buf),
           ...
           "kd: %.*f%s"
           "start i: %.*f%s\n",
           used_pid_tuner_pids->at(column).name.c_str(),
           util::places_after_decimal(kp, 2), kp, row == 0 ? a : "\n",
           util::places_after_decimal(ki, 2), ki, row == 1 ? a : "\n",
           util::places_after_decimal(kd, 2), kd, row == 2 ? a : "\n",
           util::places_after_decimal(starti, 2), starti, row == 3 ? a : "\n");
  complete_pid_tuner_output = buf;
...
void Drive::pid_tuner_print_brain() {
  if (pid_tuner_lcd_b) light::screen_print(complete_pid_tuner_output);
}
