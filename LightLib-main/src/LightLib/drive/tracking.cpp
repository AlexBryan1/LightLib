...
void Drive::odom_pose_set(pose itarget) {
  ...
  if (light::g_pose_setter) light::g_pose_setter(itarget);
}
...
double Drive::odom_x_get() { return light::g_pose_getter ? light::g_pose_getter().x : odom_current.x; }
double Drive::odom_y_get() { return light::g_pose_getter ? light::g_pose_getter().y : odom_current.y; }
double Drive::odom_theta_get() { return light::g_pose_getter ? light::g_pose_getter().theta : odom_current.theta; }
pose Drive::odom_pose_get() { return light::g_pose_getter ? light::g_pose_getter() : odom_current; }
double Drive::drive_width_get() { return global_track_width; }

std::pair<float, float> Drive::decide_vert_sensor(light::tracking_wheel* tracker, bool is_tracker_enabled, float ime, float ime_track) {
  float current = ime;
...
light::pose Drive::solve_xy_vert(float p_track_width, float current_t, float delta_vert, float delta_t) {
...
light::pose Drive::solve_xy_horiz(float p_track_width, float current_t, float delta_horiz, float delta_t) {
  pose output = {0.0, 0.0, 0.0};
...
void Drive::ez_tracking_task() {
  ...
  light::tracking_wheel* h_sensor = odom_tracker_back != nullptr ? odom_tracker_back : odom_tracker_front;
  ...
  // Angle and velocity
  float t_current = -light::util::to_rad(drive_imu_get());  // negative for math standard
