...
double to_deg(double input) { return input * light::units::DEG_PER_RAD; }
double to_rad(double input) { return input * light::units::RAD_PER_DEG; }
...
double absolute_angle_to_point(pose itarget, pose icurrent) {
  ...
  // atan2(0, 0) is implementation-defined — return 0 when the robot is at
  // the target so callers don't see NaN propagate into the heading PID.
  if (x_error == 0.0 && y_error == 0.0) return 0.0;
...
double turn_shortest(double target, double current, bool print) {
  ...
  double new_target = current + wrap_angle(target - current);
  if (print) printf("%.2f\n", new_target);
  return new_target;
}
